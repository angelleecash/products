...
namespace irr
{
    namespace video
    {
        ...
        bool COGLES2FixedPipelineShader::OnRender( IMaterialRendererServices* service, E_VERTEX_TYPE vtxtype )
        {
            ...
            //SUniformInfo& ui = UniformInfo[USE_TEXTURE];
            
            //__android_log_print(ANDROID_LOG_INFO, "USE_TEXTURE", "------------------------- USE TEXTURE LOCATION %d", ui.location);
            //UseTexture[0] = 1;
            setUniform( USE_TEXTURE, UseTexture, MAX_TEXTURE_UNITS );
            
            GLint b1location = glGetUniformLocation( Program, "b1" );
            __android_log_print(ANDROID_LOG_INFO, "FUCK ME", "b1 location = %d", b1location);
            
            //GLint v[] = {0,1,1,1};
            //glUniform1iv(b1location, 4, v);
