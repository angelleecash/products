	...
	if (nodeSydney)
	        {
                    ...
                    stringc texturePath = gSdCardPath+sydneyTextureFilename;
                    ITexture* texture = driver->getTexture((texturePath).c_str());
                    __android_log_print(ANDROID_LOG_INFO, "Irrlicht", "texture %s result %p", texturePath.c_str(), texture);
...
{	
	...
	smgr->drawAll();
	guienv->drawAll();
    nodeSydney->setRotation(vector3df(0,counter%360,0));
