...
namespace irr
{
    namespace video
    {
                                                       ...
                                                       bool registerMaterial )
        {
			...
			__android_log_print(ANDROID_LOG_INFO, "SHADER INFO", "============= %s %s %d", vertexShaderFile, pixelShaderFile, Program);
        }
        ...
        bool COGLES2SLMaterialRenderer::linkProgram()
        {
            ...
            if ( maxlen == 0 && num != 0 )
            {
                ...
                return false;
					...
					__android_log_print(ANDROID_LOG_INFO, "UNIFORM", "UNIFORM %d %d %d", Program, num, maxlen);
            ...
            for ( int i = 0; i < num; ++i )
            {
                ...
                SUniformInfo ui;
                int length = 0;
                glGetActiveUniform( Program, i, maxlen, &length, &size, &ui.type, reinterpret_cast<char*>( buf ) );
                ...
                names.push_back( buf );
                __android_log_print(ANDROID_LOG_INFO, "UNIFORM", "UNIFORM INFO program=%d index=%d name=%s type=%d size=%d length=%d oldname=%s %s", Program, i, buf, ui.type, size, length,a.c_str(), ui.location==-1?"<-----------------":"");
            }
            ...
            for ( int i = 0; i < UniformCount; ++i )
            {
                ...
                __android_log_print(ANDROID_LOG_INFO, "FINAL UNIFORM", "******************************************************** %d %d %s %d %s", Program, i, UniformStringTable[i], UniformInfo[UniformInfo.size()-1].location, UniformInfo[UniformInfo.size()-1].location==-1?"<-----------------":"" );
            }
        ...
        {
            if(index >= UniformInfo.size())
            {
                __android_log_print(ANDROID_LOG_INFO, "SET UNIFORM ERRO", "OUT OF BOUNDS -------------------------------------- %d", index);
                return false;
            }
            ...
            if (Driver->testGLError()) {
                __android_log_print(ANDROID_LOG_INFO, "SET UNIFORM ERRO", "-------------------------------------- %d", index);
            }
